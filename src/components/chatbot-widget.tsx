          ...
          {!open && visible && (
            ...
            >
              ...
              >
                ...
                <p className="mt-1.5 font-display text-[15px] font-extrabold leading-tight tracking-tight text-foreground">
                  Ask Nyay
                </p>
              ...
              >
                <img
                  src={nyayOwl.src}
                  alt="Nyay assistant"
                  className="h-[100px] w-[100px] object-contain drop-shadow-[0_4px_12px_rgba(0,0,0,0.15)]"
                />
              </button>
            </motion.div>
          )}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative shrink-0">
                      <img
                        src={nyayOwl.src}
                        alt=""
                        className="h-12 w-12 rounded-full object-cover shadow-[0_2px_8px_rgba(0,0,0,0.12)]"
                      />
                      <span className="absolute bottom-0 right-0 size-3 rounded-full bg-emerald-500 border-[2.5px] border-card" />
                    </div>
                    <div>
                      <p className="font-display text-[15px] font-bold tracking-tight text-foreground">
                        Ask Nyay
                      </p>
                      <p className="text-[11px] text-muted-foreground">AI Legal Guide</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setOpen(false)}
                      ...
                      >
                        {m.role === "model" && (
                          <img
                            src={nyayOwl.src}
                            alt=""
                            className="mt-0.5 size-8 shrink-0 rounded-full object-cover shadow-[0_1px_4px_rgba(0,0,0,0.1)]"
                          />
                        )}
                        <div
                          className={`max-w-[82%] rounded-2xl px-4 py-3 text-[16px] leading-[1.6] ${
                            m.role === "user"
                        ...
                        <div className="flex gap-2.5 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-none">
                          {QUICK_ACTIONS.map((action) => (
                            <button
                              key={action}
                              onClick={() => send(action)}
                              className="shrink-0 rounded-full border border-border bg-background px-5 py-3 text-[15px] font-medium text-foreground transition-all duration-200 hover:border-foreground/25 hover:bg-muted active:scale-95 min-h-[48px]"
                            >
                              {action}
                            </button>
                          ))}
                ...
                <div className="border-t border-border px-4 py-3 shrink-0">
                  <div className="flex items-center gap-2.5 rounded-2xl border border-border bg-background px-4 py-2 transition-colors focus-within:border-foreground/20">
                    <input
                      ...
                      className="flex-1 bg-transparent text-[16px] text-foreground placeholder:text-muted-foreground outline-none py-1"
                    />
                    <button
                      onClick={() => send()}
                      disabled={loading || !input.trim()}
                      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-foreground text-background transition-all duration-200 hover:brightness-110 active:scale-90 disabled:opacity-30 disabled:hover:brightness-100 disabled:active:scale-100"
